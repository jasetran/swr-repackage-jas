...
const midBlockTrialsContent = [
  {
    stimulus: () => {
      if (store.session.get("config").story) {
        return mediaAssets.audio.midBlock1 } // TO DO: replace audio file with AI for no story mode
    ...
    prompt: () => {
      if (store.session.get("config").story) {
        ...
        <div> <p class="center" style="position: relative; top: 50%; margin-bottom: 1.5rem;">${i18next.t(
          "gameBreakTrials.midBlockTrials.trial1.paragraph1")}</p>
          <p class="center" style="position: relative; top: 50%;">${i18next.t(
            "gameBreakTrials.midBlockTrials.trial1.paragraph2")}</p>  
            </div>
            ...
            ${!isTouchScreen ? `<div class="button">${i18next.t("navigation.continueButtonText", {
              action: `${i18next.t("terms.continue")}`})}</div>`: "" }` } // TO DO: replace wizard gif with different gif
  ...
  },
  {
    stimulus: () => {
      ...
      return mediaAssets.audio.midBlock2Ns // TO DO: replace audio file with AI for no story mode
    },
    prompt: () => {
      if (store.session.get("config").story) {
        return `
        ...
        <p class="center" style="position: relative; top: 50%; margin-bottom: 1.5rem;">${i18next.t(
          "gameBreakTrials.midBlockTrials.trial2.paragraph1")}</p>
        <p class="center" style="position: relative; top: 50%;">${i18next.t(
          "gameBreakTrials.midBlockTrials.trial2.paragraph2")}</p>
          </div>
          <div class="story-scene"><img class="scene" src="${mediaAssets.images.valley4}" alt="background with hills and trees">
          <img class = 'adventure_mid_break' src="${mediaAssets.images.adventurer1}" alt="adventurer with harp">
    ...
    },
  {
    stimulus: () => {
      ...
      return mediaAssets.audio.midBlock3Ns // TO DO: replace audio file with AI for no story mode
    }, 
    prompt: () => {
      if (store.session.get("config").story) {
        return `
        ...
        <div>
        <p class="center" style="position: relative; top: 50%; margin-bottom: 1.5rem;">${i18next.t(
          "gameBreakTrials.midBlockTrials.trial3.paragraph1")}</p>
        <p class="center" style="position: relative; top: 50%;">${i18next.t(
          "gameBreakTrials.midBlockTrials.trial3.paragraph2")}</p>
          </div>
          <div class="story-scene">
...
const postBlockTrialsContent = [
  {
    stimulus: () => {
      ...
      return mediaAssets.audio.endBlock1Ns // TO DO: replace audio file with AI for no story mode
    },
    prompt: () => {
      if (store.session.get("config").story) {
        return ` 
  ...
  },
  {
    stimulus: () => {
      ...
      return mediaAssets.audio.endBlock2Ns // TO DO: replace audio file with AI for no story mode
    },
    prompt: () => {
      if (store.session.get("config").story) {
        return `
...
const final_page = {
  ...
  stimulus: () => {
    ...
    return mediaAssets.audio.endGameNs // TO DO: replace audio file with AI for no story mode
  }, 
  keyboard_choices: () => (isTouchScreen ? "NO_KEYS" : "ALL_KEYS"),
  button_choices: () => (isTouchScreen ? ["HERE"] : []),
  button_html: () =>
  ...
  prompt: () => { 
    if (store.session.get("config").story) {
      ...
      ${ !isTouchScreen ? `<div class="button">${i18next.t("navigation.continueButtonText", {
          action: `${i18next.t("terms.save")}`})}</div>`: ""
        }`
    } // TO DO: replace wizard gif with different gif
